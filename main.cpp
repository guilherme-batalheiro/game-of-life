...
void initializeCellMap(std::vector<std::vector<myObjs::Cell>> *cellMap) {
    for (int i = 0; i < COLUMS_NUMBER; i++) {
        for (int j = 0; j < ROWS_NUMBER; j++) {
            ...
            if ((i % 2 == 0 || j % 2 == 0) && rand() % 2 == 0) (*cellMap)[i][j].revive();
        }
    }
}
...
std::vector<std::vector<myObjs::Cell>> setNextGenMap(
    ...
    std::function<void(std::vector<std::vector<myObjs::Cell>> *,
                       std::vector<std::vector<myObjs::Cell>> *, int)>
        func = [](std::vector<std::vector<myObjs::Cell>> *currCellMap,
                  std::vector<std::vector<myObjs::Cell>> *nextCellMap, int line) {
            for (int i = 0; i < ROWS_NUMBER; i++) {
                int neighbors = 0;
                for (int j = -1; j <= 1; j++) {
                    for (int k = -1; k <= 1; k++) {
                        if (!(j == 0 && k == 0) && line + j >= 0 && line + j < COLUMS_NUMBER &&
                            i + k >= 0 && i + k < ROWS_NUMBER &&
...
int main() {
    ...
    std::vector<std::vector<myObjs::Cell>> currentGen(COLUMS_NUMBER,
                                                      std::vector<myObjs::Cell>(ROWS_NUMBER));
    initializeCellMap(&currentGen);

    std::vector<std::vector<myObjs::Cell>> nextGen(COLUMS_NUMBER,
                                                   std::vector<myObjs::Cell>(ROWS_NUMBER));
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::R)) initializeCellMap(&currentGen);
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::P)) paused = true;
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::C)) paused = false;
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Delete)) cleanCellMap(&currentGen);
            if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
                placeCellInMap(sf::Mouse::getPosition(window), &currentGen);
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::K))
                killCellInMap(sf::Mouse::getPosition(window), &currentGen);
        }
        ...
        if (!paused) currentGen = setNextGenMap(&currentGen, &nextGen);
